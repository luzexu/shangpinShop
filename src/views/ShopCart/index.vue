...
export default {
  ...
  computed: {
    ...
    isAllChecked() {
      let a = this.cartInfoList.every((item) => item.isChecked == 1)
      return a
    },
  },
  methods: {
    ...
    async updateAllCartChecked(event) {
      try {
        console.log(
          'event.target.checkedevent.target.checked',
          event.target.checked
        )
        let isChencked = event.target.checked ? '1' : '0'
        console.log('isChencked', isChencked)
        // 派发action
        await this.$store.dispatch('updateAllCartChecked', isChencked)
        this.getData()
      } catch (error) {
        error.message
      }
